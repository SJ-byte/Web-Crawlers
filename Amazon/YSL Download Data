    ...
    for li in lis:
        ...
        if temp is not None:
            ...
            if temp1 is not None:
                ...
                #print(prev_div)
                try: 
                    notes = (temp.find_parents("span")[0].find_parents("span")[0].find_parents("a")[0].find_parents("div")[0].find_parents("div")[0].next_sibling.string)
                except:
                    notes = ("No Notes")
                ...
                if m:
                    name = (m.group(1))
                    tempSer = pd.DataFrame({"Name" : [name], "Notes" : [notes]},index = [price])
                    tore = pd.concat([tore,tempSer])
                    #print(tore)
                else:
...
tore.to_csv(name)
import AmazonResult
#os.system('/Users/sunjiaxuan/Documents/Py/AmazonResult.py')
